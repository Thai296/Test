...
//			searchDATTF.typeKeys(sNumeroSS);
			...
			sleepSilently(10000);
			LOGGER.info("Checking after sleep");
//			new CustomWebDriverWait(browser, 10).until(ExpectedConditions.presenceOfElementLocated(By.xpath(xpathNew)));
			ControlledElement sNumeroSSEle = new ControlledElement(browser, By.xpath(xpathNew));
			found.set(sNumeroSSEle.isFound());
			LOGGER.info("Wait for data is found");
			if (!found.get()) {
...
//			Utility.waitForElement(browser, 10, ExpectedConditions.invisibilityOfElementLocated(By.xpath(DAT_SAVED_SUCCESS_MESSAGE)));
	...
	public boolean downloadDATZIP(String sFileName) {
		try {
			return (boolean) downloadDATZIPWithFiles(sFileName).get("downloaded");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return false;
...
//		String resourcePath = getResourcePath("");
	...
	public void importDATF(String sFileName, String succVerificationXPath) {
		...
		new CustomWebDriverWait(browser, 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(succVerificationXPath)));
